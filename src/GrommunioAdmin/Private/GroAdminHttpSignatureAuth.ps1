...
.OUTPUTS
    Hashtable
#>
function Get-GroAdminHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-GroAdminConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-GroAdminStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-GroAdminUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-GroAdminUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-GroAdminStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-GroAdminKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-GroAdminRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-GroAdminECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Depricated
    <#$cryptographicScheme = Get-GroAdminCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
.OUTPUTS
    Base64String
#>
function Get-GroAdminRSASignature {
    Param(
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "GroAdminRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-GroAdminECDSASignature {
    param(
...
.Outputs
    String
#>
Function Get-GroAdminStringHash {
    param(
        [Parameter(Mandatory = $true)]
        [AllowEmptyString()]
        [string]$String,
...
function Get-GroAdminCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-GroAdminKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
