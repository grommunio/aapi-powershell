...
PasswdRequest<PSCustomObject>
#>

function Initialize-GroAdminPasswdRequest {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: GrommunioAdmin => GroAdminPasswdRequest' | Write-Debug
...
Convert from JSON to PasswdRequest<PSCustomObject>
...
PasswdRequest<PSCustomObject>
#>
function ConvertFrom-GroAdminJsonToPasswdRequest {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: GrommunioAdmin => GroAdminPasswdRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in GroAdminPasswdRequest
