...
UserInit<PSCustomObject>
#>

function Initialize-GroAdminUserInit {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: GrommunioAdmin => GroAdminUserInit' | Write-Debug
...
Convert from JSON to UserInit<PSCustomObject>
...
UserInit<PSCustomObject>
#>
function ConvertFrom-GroAdminJsonToUserInit {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: GrommunioAdmin => GroAdminUserInit' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in GroAdminUserInit
        $AllProperties = ('username', 'password', 'domainID', 'properties', 'altnames', 'aliases', 'status', 'pop3_imap', 'smtp', 'changePassword', 'publicAddress', 'privChat', 'privVideo', 'privFiles', 'privArchive', 'privWeb', 'privEas', 'privDav', 'fetchmail', 'syncPolicy', 'chat', 'chatAdmin', 'lang', 'forward', 'homeserver')
