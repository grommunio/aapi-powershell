...
function ConvertFrom-GroAdminJsonToUser {
    ...
    Process {
        ...
        $AllProperties = ('ID', 'username', 'domainID', 'roles', 'properties', 'altnames', 'aliases', 'status', 'pop3_imap', 'smtp', 'changePassword', 'publicAddress', 'privChat', 'privVideo', 'privFiles', 'privArchive', 'privWeb', 'privEas', 'privDav', 'ldapID', 'fetchmail', 'syncPolicy', 'chat', 'chatAdmin', 'lang', 'forward', 'homeserver', 'mlist', 'orgID')
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match 'mlist'))) { #optional property not found
            $Mlist = $null
        } else {
            $Mlist = $JsonParameters.PSobject.Properties['mlist'].value
        }
